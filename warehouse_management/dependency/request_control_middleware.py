...
from warehouse_management.api.v1.routes import router_list
from warehouse_management.core.config import configs
from warehouse_management.core.container import Container
from warehouse_management.dependency.user_token import get_user_by_token
from warehouse_management.enum.log_status_enum import LogStatusEnum
from warehouse_management.schema.server_logs_schema import ServerLogsUpsert
...
class RequestControlMiddleware(BaseHTTPMiddleware):
    ...
    async def create_audit_log(
        ...
        log_data.status = LogStatusEnum.success
        ...
        for router in router_list:
            for route in router.routes:
                if request_path[-1].isdigit():
                    ...
                    request_path[-1] = "{id}"
                if isinstance(route, APIRoute) and (configs.API_V1_STR + route.path) == "/".join(request_path):
